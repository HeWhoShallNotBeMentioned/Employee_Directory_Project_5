(function(window) {
  ...
  function randomUserData(data, textStatus, jqXHR) {
    ...
    data.results.forEach(function(item, index, arr) {
      ...
      addHtml +=
        ...
        index +
        '" src="' +
        data.results[index].picture.thumbnail +
        '"></div>';
      addHtml += '<div>';
      addHtml +=
        '<span class="name ' +
  ...
  function eachEmployee() {
    ...
    for (var k = 0; k < searchData.length; k++) {
      ...
      if (
        (firstNameSearch.toLowerCase().indexOf(filter) > -1 ||
          lastNameSearch.toLowerCase().indexOf(filter) > -1 ||
          usernameSearch.toLowerCase().indexOf(filter)) > -1
      ) {
  ...
  $.ajax({
    ...
    success: function(data, textStatus, jqXHR) {
      ...
      document
        ...
        .addEventListener('keyup', eachEmployee);
    },
    error: function() {
